...
#define LINE_TOKEN  "ymT6uxf9tilFQaPtrgQ3Efa3tDOMwLdmQBppNOM9d9d" //NBf27NXc0PwNcdHyLDP9fWEqfPbRoxT3LEM0Qr0lyFo
...
unsigned long samplingTime, time2, time3;
unsigned long runTimeBegin = 0;
unsigned long runTime;
bool startState = false;

//DISPLAY VARIBLE
String msg1,msg2,msg3,msg4,msg5,msg6,spMSG;
String msgRadio;
int val;
...
void loop()
{
  ...
  if(current > 45 && current < 9999){                               // 9999 is returned value when RF was disconnected!
    lcd.backlight();
    lcd.clear();
    startState = true;
    runTimeBegin = millis();
    avgIndex = 0;
    lcdSingle("  MOTOR START!  ",0,0);
    Serial.println(F("MOTOR START!"));
    delay(7000);                                                     // Delay for Start Current 7Sec
    lcd.clear();
    ...
    while(startState == true)                                        //start State at RATED current
    {
      ...
      //Vibration Test สำหรับไม่ได้ต่อ Sensor
    ...
    //GET AVERAGE VALUE                                             //หาค่าเฉลี่ยของ Vibration และ Current
      if(millis() - samplingTime >= 1000){
        ...
        if(current < 45){                                           //ออกจาก  While loop เมื่อกระแสต่ำกว่า 20A
          startState =  false; 
        }
...
void Display(){
    ...
    msg6 = "Time:  " + String((millis() - runTimeBegin) / 1000) + " SEC";
...
void notice(){
  if(peakx >= 70 || peaky >= 70 >= peakz >= 70){
    LINE.notify("Critical Vibration!");
    LINE.notifySticker(1,3);
    Serial.println("Critical Vibration!");
    delay(1000);
  }
  if(current >= 700){
    LINE.notify("Critical Current!");
    LINE.notifySticker(1,3);
    Serial.println("Critical Current!");
    delay(1000);
  }
}
